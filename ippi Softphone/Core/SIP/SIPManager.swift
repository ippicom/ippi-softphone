...
final class SIPManager: SIPManagerProtocol {
    ...
    // Call health monitor — periodic stats logging during active calls
    private var callMonitorTask: Task<Void, Never>?
    private static let callMonitorInterval: Duration = .seconds(30)
    ...
    private func handleNetworkChange(_ path: NWPath) {
        ...
        Log.sip.info("Network interfaces: [\(interfaces.joined(separator: ", "))], constrained=\(path.isConstrained), expensive=\(path.isExpensive)")

        // If there's an active call, log its stats after network change
        if hasActiveCall {
            Log.sip.warning("Network changed DURING active call — logging call stats")
            for (_, wrapper) in calls {
                let callId = wrapper.lpCall.callLog?.callId ?? "?"
                logCallStats(wrapper.lpCall, callId: callId)
            }
        }
    }
    ...
    private func handleCallStateChanged(_ linphoneCall: LPCall, state: LPCallState) {
        ...
        case .Connected, .StreamsRunning:
            ...
            logCallMediaDetails(linphoneCall, state: state)
            startCallMonitorIfNeeded()
        case .Paused, .Pausing:
            call.state = .paused
            call.isOnHold = true
            logCallMediaDetails(linphoneCall, state: state)
        ...
        case .PausedByRemote:
            ...
            Log.sip.warning("Call PAUSED BY REMOTE — remote party put us on hold")
            logCallMediaDetails(linphoneCall, state: state)
        case .Error:
            call.state = .error
            call.endTime = Date()
        ...
        case .End:
            ...
            logCallMediaDetails(linphoneCall, state: state)
            stopCallMonitorIfNoActiveCalls()
    ...
    // MARK: - Call Health Monitor

    /// Log media details for a call (codec, encryption, stats) — called on state transitions.
    /// Only logs in verbose mode to avoid overhead in production.
    private func logCallMediaDetails(_ lpCall: LPCall, state: LPCallState) {
        guard LogFileManager.shared.isVerboseLogging else { return }
        let callId = lpCall.callLog?.callId ?? "?"

        // Current audio parameters
        let params = lpCall.currentParams
        let codec = params?.usedAudioPayloadType?.mimeType ?? "none"
        let clockRate = params?.usedAudioPayloadType?.clockRate ?? 0
        let channels = params?.usedAudioPayloadType?.channels ?? 0
        let encryption = params?.mediaEncryption
        let encName: String
        switch encryption {
        case .SRTP: encName = "SRTP"
        case .ZRTP: encName = "ZRTP"
        case .DTLS: encName = "DTLS"
        case .None: encName = "none"
        default:    encName = "unknown(\(encryption?.rawValue ?? -1))"
        }

        Log.sip.info(
            "Call media [\(state)]: callId=\(callId), codec=\(codec)/\(clockRate)/\(channels)ch, "
            + "encryption=\(encName), audioEnabled=\(params?.audioEnabled ?? false)"
        )

        // RTP stats (available when call has been active)
        logCallStats(lpCall, callId: callId)
    }

    /// Log RTP/RTCP statistics for a call
    private func logCallStats(_ lpCall: LPCall, callId: String) {
        guard let stats = lpCall.getStats(type: .Audio) else {
            Log.sip.info("Call stats [\(callId)]: no audio stats available")
            return
        }

        let uploadBw = stats.uploadBandwidth   // kbits/s
        let downloadBw = stats.downloadBandwidth
        let senderLoss = stats.senderLossRate
        let receiverLoss = stats.receiverLossRate
        let senderInterarrivalJitter = stats.senderInterarrivalJitter
        let receiverInterarrivalJitter = stats.receiverInterarrivalJitter
        let roundTripDelay = stats.roundTripDelay
        let iceState = stats.iceState

        let iceName: String
        switch iceState {
        case .NotActivated:      iceName = "notActivated"
        case .Failed:            iceName = "failed"
        case .InProgress:        iceName = "inProgress"
        case .HostConnection:    iceName = "hostConnection"
        case .ReflexiveConnection: iceName = "reflexiveConnection"
        case .RelayConnection:   iceName = "relayConnection"
        @unknown default:        iceName = "unknown(\(iceState.rawValue))"
        }

        Log.sip.info(
            "Call stats [\(callId)]: "
            + "up=\(String(format: "%.1f", uploadBw))kbps, down=\(String(format: "%.1f", downloadBw))kbps, "
            + "sndLoss=\(String(format: "%.1f", senderLoss))%, rcvLoss=\(String(format: "%.1f", receiverLoss))%, "
            + "sndJitter=\(String(format: "%.1f", senderInterarrivalJitter))ms, rcvJitter=\(String(format: "%.1f", receiverInterarrivalJitter))ms, "
            + "rtt=\(String(format: "%.0f", roundTripDelay))ms, ice=\(iceName)"
        )
    }

    /// Start periodic call stats monitor when a call becomes active.
    /// Only runs in verbose logging mode (VERBOSE_LOGGING build or runtime debug mode).
    private func startCallMonitorIfNeeded() {
        guard LogFileManager.shared.isVerboseLogging else { return }
        guard callMonitorTask == nil else { return }

        callMonitorTask = Task { [weak self] in
            var tick = 0
            while !Task.isCancelled {
                try? await Task.sleep(for: Self.callMonitorInterval)
                guard !Task.isCancelled, let self else { break }

                tick += 1
                let elapsed = tick * Int(Self.callMonitorInterval.components.seconds)

                // Log stats for all active calls
                for (uuid, wrapper) in self.calls {
                    let callId = wrapper.lpCall.callLog?.callId ?? uuid.uuidString.prefix(8).description
                    Log.sip.info("Call monitor [t+\(elapsed)s]: uuid=\(uuid.uuidString.prefix(8))…")
                    self.logCallStats(wrapper.lpCall, callId: callId)

                    // Also log audio stream state from linphone
                    let params = wrapper.lpCall.currentParams
                    let audioEnabled = params?.audioEnabled ?? false
                    let callState = wrapper.lpCall.state
                    let mediaInProgress = wrapper.lpCall.mediaInProgress()
                    Log.sip.info(
                        "Call state [t+\(elapsed)s]: callId=\(callId), lpState=\(callState), "
                        + "audioEnabled=\(audioEnabled), mediaInProgress=\(mediaInProgress)"
                    )
                }

                // Log audio session state
                await MainActor.run {
                    AppEnvironment.shared.audioManager.logAudioSessionState()
                }
            }
        }
        Log.sip.info("Call health monitor started (interval=\(Self.callMonitorInterval))")
    }

    /// Stop call monitor when no more active calls
    private func stopCallMonitorIfNoActiveCalls() {
        // Count non-ended calls
        let activeCount = activeCalls.values.filter { $0.state != .ended && $0.state != .error }.count
        guard activeCount == 0 else { return }

        callMonitorTask?.cancel()
        callMonitorTask = nil
        Log.sip.info("Call health monitor stopped")
    }
    ...
    /// Update liblinphone log level based on verbose logging state
    nonisolated private func updateLinphoneLogLevel() {
        guard let loggingService = loggingServiceRef else { return }

        if LogFileManager.shared.isVerboseLogging {
            loggingService.logLevelMask = Self.traceLogMask
            Log.sip.call("Liblinphone log level set to TRACE (verbose)")
        } else {
            loggingService.logLevelMask = Self.errorLogMask
            Log.sip.call("Liblinphone log level set to ERROR (normal)")
        }
    }
