...
final class AudioSessionManager {
    ...
    private func setupNotifications() {
        ...
        notificationObservers.append(
            NotificationCenter.default.addObserver(
                ...
                MainActor.assumeIsolated {
                    Log.audio.failure("Media services were LOST — audio hardware reset by system")
                }
            }
        )
        ...
        notificationObservers.append(
            NotificationCenter.default.addObserver(
                ...
                MainActor.assumeIsolated {
                    Log.audio.warning("Media services were RESET — need to reconfigure audio session")
        ...
        notificationObservers.append(
            NotificationCenter.default.addObserver(
                ...
                MainActor.assumeIsolated {
                    let type = (notification.userInfo?[AVAudioSessionSilenceSecondaryAudioHintTypeKey] as? UInt)
                        .flatMap { AVAudioSession.SilenceSecondaryAudioHintType(rawValue: $0) }
                    Log.audio.info("Secondary audio hint: \(type == .begin ? "BEGIN silence" : type == .end ? "END silence" : "unknown")")
                }
            }
        )
    }
    ...
    private func handleRouteChange(_ notification: Notification) {
        ...
        @unknown default:              reasonName = "rawValue(\(reason.rawValue))"
        }

        let prevRoute = (userInfo[AVAudioSessionRouteChangeReasonKey + "PreviousRoute"] as? AVAudioSessionRouteDescription)
            ?? (userInfo["AVAudioSessionRouteChangePreviousRouteKey"] as? AVAudioSessionRouteDescription)
        let prevOutputs = prevRoute?.outputs.map { "\($0.portType.rawValue)(\($0.portName))" }.joined(separator: ",") ?? "?"
        let currentOutputs = audioSession.currentRoute.outputs.map { "\($0.portType.rawValue)(\($0.portName))" }.joined(separator: ",")
        let currentInputs = audioSession.currentRoute.inputs.map { "\($0.portType.rawValue)(\($0.portName))" }.joined(separator: ",")

        Log.audio.info("Route changed: reason=\(reasonName), prev=[\(prevOutputs)], now=out[\(currentOutputs)] in[\(currentInputs)]")
        updateCurrentRoute()
    }
    ...
    private func handleInterruption(_ notification: Notification) {
        ...
        switch type {
        case .began:
            Log.audio.warning("Audio interruption BEGAN (reason=\(reason))")
            logAudioSessionState()
        case .ended:
            if let optionsValue = userInfo[AVAudioSessionInterruptionOptionKey] as? UInt {
                let options = AVAudioSession.InterruptionOptions(rawValue: optionsValue)
                let shouldResume = options.contains(.shouldResume)
                Log.audio.info("Audio interruption ENDED (shouldResume=\(shouldResume))")
                if shouldResume {
                    try? activateSession()
                }
            } else {
                Log.audio.info("Audio interruption ENDED (no options)")
            }
            logAudioSessionState()
        @unknown default:
            Log.audio.warning("Audio interruption unknown type: \(typeValue)")
        }
    }
    
    /// Dump current audio session state for diagnostics
    func logAudioSessionState() {
        ...
        let ioBuffer = audioSession.ioBufferDuration
        ...
        Log.audio.info(
            "AudioSession state: cat=\(category), mode=\(mode), "
            + "sampleRate=\(sampleRate)Hz, ioBuf=\(String(format: "%.1f", ioBuffer * 1000))ms, "
            + "inLat=\(String(format: "%.1f", inputLatency * 1000))ms, outLat=\(String(format: "%.1f", outputLatency * 1000))ms, "
            + "out=[\(outputs)], in=[\(inputs)], otherPlaying=\(isOtherPlaying)"
        )
    }
