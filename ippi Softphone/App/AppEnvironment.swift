...
final class AppEnvironment {
    ...
    /// Periodic task that logs audio session state during the ringing phase,
    /// to diagnose reports of silent ringing with Bluetooth A2DP routes.
    private var ringingDiagnosticsTask: Task<Void, Never>?
    #endif
    ...
    #if os(iOS)
    /// Start periodic audio-state logging during the ringing phase.
    /// Only runs when verbose logging is enabled — gated to avoid prod log spam.
    /// Used to diagnose silent-ringtone reports on Bluetooth A2DP routes.
    func startRingingDiagnostics() {
        guard LogFileManager.shared.isVerboseLogging else { return }
        ringingDiagnosticsTask?.cancel()
        Log.audio.info("Ringing diagnostics: start — should be audible from now on")
        audioManager.logAudioSessionState()
        ringingDiagnosticsTask = Task { @MainActor [weak self] in
            var tick = 1
            while !Task.isCancelled {
                try? await Task.sleep(for: .seconds(2))
                if Task.isCancelled { break }
                Log.audio.info("Ringing diagnostics tick \(tick) — still expecting audible ringtone")
                self?.audioManager.logAudioSessionState()
                tick += 1
            }
        }
    }

    func stopRingingDiagnostics() {
        guard ringingDiagnosticsTask != nil else { return }
        ringingDiagnosticsTask?.cancel()
        ringingDiagnosticsTask = nil
        Log.audio.info("Ringing diagnostics: stop")
    }
    #endif
