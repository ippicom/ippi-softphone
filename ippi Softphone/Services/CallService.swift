...
final class CallService: CallKitActionDelegate {
    ...
    func handleProviderReset() {
        ...
        // NOTE: do NOT clear environment.pendingIncomingCall here.
        // CallKit fires providerDidReset at cold start (before any push is
        // processed), and the handler runs async on MainActor — by the time
        // it executes, a VoIP push may already have set the pending UUID.
        // Wiping it here causes resolveCall to mint a new UUID that doesn't
        // match the one CallKit is ringing → ghost call. The pending state
        // has its own TTL (30s) and is managed by the PushKit→SIP flow.
    }
