...
final class LogFileManager: Sendable {
    ...
    /// Verbose logging is active when compiled with VERBOSE_LOGGING (TestFlight builds)
    /// or when the user activates debug mode manually (production builds).
    nonisolated var isVerboseLogging: Bool {
        #if VERBOSE_LOGGING
        return true
        #else
        return isDebugMode
    ...
    nonisolated func enableDebugMode() {
        ...
        // Skip clearing when VERBOSE_LOGGING is active (TestFlight builds) — logs are
        // already being written continuously and clearing would destroy useful diagnostics.
    ...
    ///
    /// Filtering rules:
    /// - Normal mode: only write warning + error level
    /// - Verbose mode (VERBOSE_LOGGING build flag or runtime debug mode):
    ///   write info, notice, warning, error (and SIP protocol logs)
    ...
    nonisolated func write(_ message: String, category: String, level: LogLevel) {
        ...
        case .info, .notice, .warning:
            // Only write in verbose mode (compile-time flag or runtime debug)
            shouldWrite = isVerboseLogging
        case .error:
            // Always write
            shouldWrite = true
        }
    ...
    private func createExportHeader() -> String {
        let verboseFlag: String
        #if VERBOSE_LOGGING
        verboseFlag = "VERBOSE_LOGGING=ON (build flag)"
        #else
        verboseFlag = "VERBOSE_LOGGING=OFF"
        #endif
        ...
        \(deviceInfo())
        \(appInfo())
        Logging: \(verboseFlag), debugMode=\(isDebugMode)
        =============================================
        """
    }
