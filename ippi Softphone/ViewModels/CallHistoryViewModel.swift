...
final class CallHistoryViewModel {
    ...
    func callBack(_ entry: CallHistoryEntry) async {
        do {
            ...
            // Service/feature codes (*…/#…) and usernames (with @) are ready as-is;
            // plain phone numbers need E.164 normalization.
            // (normalize() strips non-digits and would drop the * / #.)
            let destination: String
            if dialAddress.contains("@") || dialAddress.contains("*") || dialAddress.contains("#") {
                destination = dialAddress
            } else {
                destination = PhoneNumberFormatter.normalize(dialAddress)
            }
